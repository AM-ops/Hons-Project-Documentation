...
From Set Theory using a 2-Dimensional grid or array the Adjacency of any two "squares" or "cells" in the grid can be defined as:\cite{aima}
\begin{center}
$\forall x, y, a, b$ \textit{Adjacent}$([x, y], [a, b])$\\\textit{if and only if}\\$(x = a \land (y = b - 1 \lor y = b + 1)) \lor (y = b \land (x = a - 1 \lor x = a + 1))$
\end{center}
The figure below demonstrates this rule. Given a set of [a, b] coordinates  which can be defined
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{Figures/Chapter3/adjacent}

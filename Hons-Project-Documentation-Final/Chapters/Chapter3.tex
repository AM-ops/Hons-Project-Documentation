...
The avant-garde approach which in turn is also intuitive to solve the issues of edge cases is to visualise two separate 2-Dimensional arrays. The first is of size $3 \times 3$ (labelled as $A$), and the the other is $n \times n$ (labelled as $B$) where $n$ is of arbitrary value. Thereafter, assign the center block of grid $A$ as the Current or Active block, and any of the other neighbouring blocks as the edge case to be tested. Place the Current block on top of grid $B$ and align it with the topmost-left block of grid $B$
\begin{figure}[H]
\centering
